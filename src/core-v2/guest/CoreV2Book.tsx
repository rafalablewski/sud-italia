...
export function CoreV2Book() {
  ...
  return (
    <CoreV2Shell
      ...
      tabs={guestTabs("book")}
      subRight={<input className="cv-inp" type="date" value={date} onChange={(e) => setDate(e.target.value)} style={{ height: 32 }} />}
    >
      <div className="cv-book">
        {/* form */}
        <div className="cv-book-form">
          <h4 className="cv-profile-h">Dine-in slot</h4>
          {loading ? (
            <div className="cv-ctx-empty">Loading slots…</div>
          ) : dineInSlots.length === 0 ? (
            <div className="cv-ctx-empty">No dine-in slots for this day.</div>
          ) : (
            <div className="cv-pks">
              {dineInSlots.map((s) => (
                <button key={s.id} className={slotId === s.id ? "cv-pk on" : "cv-pk"} onClick={() => setSlotId(s.id)}>
                  {s.time}
                </button>
              ))}
            </div>
          ...
          <h4 className="cv-profile-h">Party size</h4>
          <div className="cv-covers" style={{ width: "fit-content" }}>
          ...
          <h4 className="cv-profile-h">
            Table
            <button className="cv-btn ghost sm" style={{ marginLeft: "auto" }} onClick={recommend}>✨ Recommend</button>
          </h4>
          {tables.length === 0 ? (
            <div className="cv-ctx-empty">No tables configured.</div>
          ) : (
            <div className="cv-pks">
              {tables.map((t) => {
                const st = tableState(t);
                return (
                  <button key={t.id} className={`cv-pk ${tableId === t.id ? "on" : ""} ${st.ok ? "" : "off"}`} disabled={!st.ok && !override} onClick={() => setTableId(t.id)} title={st.label}>
                    {t.number}
          ...
          <h4 className="cv-profile-h">Guest</h4>
          <div className="cv-book-fields">
            <input className="cv-inp" value={name} onChange={(e) => setName(e.target.value)} placeholder="Guest name" />
            <input className="cv-inp" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+48…" />
            <input className="cv-inp" style={{ gridColumn: "1 / -1" }} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="High chair, window…" />
          </div>

          <div className="cv-book-actions">
            <label className="cv-ov">
              <input type="checkbox" checked={override} onChange={(e) => setOverride(e.target.checked)} />
              Override conflicts & capacity
            </label>
            <button className="cv-btn primary" disabled={!canBook} onClick={() => void book()}>
              ✓ Book slot + table
            </button>
          </div>
        </div>
        ...
        {/* today's bookings */}
        <aside className="cv-book-side">
          <h4 className="cv-profile-h">Booked · {todays.length}</h4>
          {todays.length === 0 ? (
            <div className="cv-ctx-empty">No bookings yet for this day.</div>
          ) : (
            <div className="cv-svc-list">
              {todays.map((r) => (
                <div className="cv-svc-res" key={r.id}>
                  <span className="t mono">{r.time}</span>
                  <div className="m">
                    <div className="nm">{r.customerName}</div>
                    <div className="cv-cust-sub">{r.partySize}p · table {tableLabel(r.tableId)}</div>
                  </div>
