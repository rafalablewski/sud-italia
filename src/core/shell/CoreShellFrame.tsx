...
import { useCoreShellChrome } from "./CoreShellContext";
 ...
 *
 * This frame is rendered ONCE by the `/core` layout (via `CoreProviders`) and
 * wraps `{children}` (the page). It never unmounts across navigation, so the
 * top bar + Lens Rail stay put while you move between pages and tabs — no
 * remount, no black flash. The active surface only feeds its *slice* of chrome
 * (eyebrow · view tabs · body sub-toolbar · bleed) through `useCoreShellChrome`,
 * published by the `CoreShell` a surface renders. Everything global here renders
 * exactly once.
 *
 * Core is a SEPARATE entity from /admin: this renders none of the admin shell.
 * It loads only the core theme (see src/app/core/layout.tsx).
 */
export function CoreShellFrame({ children }: { children: ReactNode }) {
  const chrome = useCoreShellChrome();
  const tabs = chrome?.tabs;
  const subLeft = chrome?.subLeft;
  const subRight = chrome?.subRight;
  ...
  return (
    <>
      ...
      {/* Persistent Context Dock — the selected entity's check, following the
          ...
          (additive; see SelectionContext / CoreDock). Mounted once here. */}
      <CoreDock />
      <CommandPalette />
      <CoreHandover />
    </>
