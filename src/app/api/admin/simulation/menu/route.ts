...
export const GET = withAdmin(
  ...
  async (req) => {
    ...
    // Roll up per-portion cost from each recipe + current ingredient
    // prices, mirroring the pattern in /api/admin/menu so the two
    ...
    // Popularity — sum line quantity per menu item over the last 30 days
    // (excluding cancellations). Used by the "Auto-fill" button.
    ...
    const items: MenuRow[] = menu
      ...
      .sort((a, b) => {
        // Most-ordered first, then by category, then by name.
