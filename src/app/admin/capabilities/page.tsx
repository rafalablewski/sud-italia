    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          caveats:
            "The live POS terminal at /admin/pos now consumes this plan (Steer toggle, on by default): per-category honest promise chips, ★ make-now / ▼ ease badges on product cards, a steering strip, a per-tab promise pill, and a delivery-intake-paused notice when the bottleneck can't absorb more. The objective is margin-per-bottleneck-second (textbook Theory of Constraints) and is not yet demand-weighted, so it eases a high-volume low-margin hero (e.g. Margherita) before a premium slow item — correct for yield-per-constraint, but a production version should weight by sales velocity. Promise times assume one server per station (same limitation as the Pace engine).",
        },
        {
          name: "POS terminal — multiple open checks (Tabs)",
          status: "live",
          href: "/admin/pos",
          summary:
            "Counter order-entry built for a busy truck window: a rail of concurrent open checks (\"tabs\"), each its own running total, item count, channel and status (open / parked / ready-to-pay). Tabs are server-persisted (pos-tabs.json via getPosTabs/savePosTab/deletePosTab) so several checks survive a reload and are shared across terminals at the same truck — a tab stores only menuItemId + quantity (never a price), re-priced server-side against the live menu. Each tab carries its own channel (Takeout / Delivery / Dine-in), with a real floor-table picker + covers (dine-in, conflict-flagged when another open check holds the table) or a delivery address. Combo discounts use the SAME getActiveComboDeals engine as the customer cart, so the applied discount is subtracted from the charged total, not just displayed; an order-aware offers panel surfaces combo-completion + cross-sell picks via getCartSuggestions. 'Send to KDS' fires the real Order through createOrder (notifications suppressed — the guest is at the window) so the check lands on the Kitchen Display + Orders list; re-firing an edited tab patches the existing ticket. 'Charge' opens a Cash / Card tender sheet, marks the linked order paid (or fires + pays in one step for a check never sent), then closes the tab. Native Fullscreen, a live clock, and keyboard shortcuts (N new tab, 1–9 switch, F fullscreen, Esc cancel). Reuses the KDS Pace steering plan (see above). Staff+, location-scoped. Ported from the chosen /mockups/pos/06-tabs.html direction.",
          caveats:
            "Tender method (Cash / Card) is captured for the receipt toast but not yet posted to the cash-drawer session — cash reconciliation in /admin/cash is a separate ledger. The floor-table picker calls GET /api/admin/floor/tables (manager+), so a staff-only login sees an empty picker until a manager has configured tables. Open checks poll every 8s; the refresh defers to in-flight local edits so a poll never clobbers an unsaved change.",
        },
        {
