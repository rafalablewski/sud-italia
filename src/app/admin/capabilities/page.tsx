  ...
  const groups: CapabilityGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          name: "Live-order simulation (KDS demo mode)",
          status: "live",
          href: "/admin/settings",
          summary:
            "One-toggle demo / training mode. Flip kdsSimulatorEnabled in /admin/settings and open /admin/kds — the board auto-drives itself: every few seconds it advances each visible truck's tickets through their dwell timers and trickles in a new order, so any KDS lens (owner fleet, manager floor, chef line) runs a live rush with no buttons or separate page. Orders are built ONLY from the location's real menu via getMenuWithOverrides() — no made-up products — and persisted through createSimulatedOrder(), which fires the same order-created SSE event a paid checkout does, so simulated tickets stream onto the board exactly like live ones and carry a '(sim)' customer-name tag. Every order carries simulated:true, so getOrders() filters them out of all reports / CRM / analytics by default (only the orders + kitchen routes opt in via { includeSimulated:true }) and they never trigger stock decrement, customer rollups or outbox SMS/email. The driver targets the selected truck, or every active truck when an owner is on the fleet view; the endpoint self-caps active sims per truck. Turning the toggle off purges the synthetic tickets (purge stays allowed even when disabled). Owner drives all trucks; a scoped manager drives their own.",
          caveats:
            "Off by default — flip it on at /admin/settings. Serverless has no background worker, so the simulation only advances while a KDS tab is open (it's driven client-side from the board). Drives the main orders-based KDS board; it intentionally does not fire per-station kds_tickets rows (that table has no cascade on order delete, so firing them would orphan rows on purge), so the /kitchen station + expo screens won't show simulated tickets.",
        },
        {
