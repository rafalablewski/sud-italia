  ...
  const groups: CapabilityGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          name: "Role-aware KDS — fleet + floor-control lenses",
          status: "live",
          href: "/admin/kds",
          summary:
            "One live-order KDS engine, lens by role. OWNER lands on a Fleet command view: a per-truck tile grid showing live open tickets, late / soon-due SLA counts, oldest ticket age, completed-today + revenue-today, and the slowest station's P95 bump time, plus a fleet KPI strip (open / late / trucks-behind / completed / revenue) and a green-amber-red health rail per truck. Click a truck to drop into its floor board (sets location + switches lens); a Fleet ↔ Floor toggle persists the choice. GET /api/admin/kds/fleet, owner-only, cross-location, 6s refresh. MANAGER / FRANCHISEE (and an owner drilled into a truck) get the floor board plus a floor-control header: live open / late / due-soon / oldest / average-age computed from the active orders the board already streams, with throughput (done last hour) + on-shift staff (open time-punches) + live 86 management (chip-restore + '86 an item' picker that PUTs an availability override to /api/admin/menu, audit-logged as menu.item_86) from GET /api/admin/kds/floor-ops. KITCHEN / STAFF keep the floor line board unchanged. Real-time signals reuse the same active orders the boards read (includeSimulated, so a simulator demo lights everything up); bump-time P95 reads getKdsStationAnalytics (real kds_tickets only).",
          caveats:
            "Chef line-view upgrades (my-station focus, queue depth, recall, item-level 86) and a persisted expedite/reprioritize action are the remaining work. P95 reads real station tickets, so it shows '—' during a pure-simulator demo. Throughput is orders completed-and-created in the trailing hour (no separate completedAt timestamp), which is accurate at truck prep times.",
        },
      ],
    },
    {
