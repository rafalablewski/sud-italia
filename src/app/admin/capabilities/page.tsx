    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          status: "live",
          href: "/admin/upsell",
          summary: "Per-location experiment with weighted variants and per-bundle discount overrides. Customer phone → SHA-256 bucket → stable variant assignment, so a customer always sees the same offer across visits and the server can reproduce the variant at checkout. Each BundleEvent records the variant id, so /api/admin/bundle-analytics surfaces avg paid + avg saved per variant for direct AOV / contribution-profit comparison. Default off (no experiment configured = single discount % wins). Source of truth: LocationUpsellConfig.experiment in upsell-settings.json; resolver in src/lib/experiments.ts.",
        },
        {
          name: "Scheduled bundle (weekly usual)",
          status: "needs-config",
          href: "/admin/upsell",
          summary: "Pret-style 'make this my Friday usual' intent capture. Customer opts in after applying a bundle; POST /api/customer/schedule-bundle persists a ScheduledBundleIntent with bundle id, weekday, ready-time, cart snapshot, status. Admin lists pending intents via GET /api/admin/scheduled-bundles. Phase 1 ships the data model + endpoints; Phase 2 wires Stripe Subscriptions to actually rebill on the chosen weekday (gated on STRIPE_SECRET_KEY + STRIPE_SCHEDULE_WEBHOOK_SECRET — not yet configured).",
