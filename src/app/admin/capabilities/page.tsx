    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: "/admin/menu",
          summary:
            "Menu admin is chain-wide. The /admin/menu list renders one row per product (no per-location chips, no location switcher) — values that diverge across trucks surface as a compact range (e.g. 27,90–29,90 zł) plus a 'varies' badge so operators can scan at a glance which SKUs price unevenly. Clicking edit opens /admin/menu/[baseSlug], a dedicated detail page split into three stacked cards: (1) per-location pricing — inline price + cost inputs per truck, availability toggle, hide / remove / add buttons, plus 'Apply price to all' and 'Reset overrides' quick actions; (2) chain-wide product fields (name, description, category, tags, SKU, delivery-only, packaging cost); (3) modifier matrix — group structure (label, min/max, option labels, KDS flag) propagates chain-wide while each option's priceDelta + costDelta diverge per location, side-by-side in a horizontally-scrolling table so a +Sourdough surcharge in Szczecin can land at +5 zł while Lublin charges +3. Saves route per-variant: seed rows batch through PUT /api/admin/menu items map (modifierGroups round-trip per row); custom rows hit PATCH /api/admin/menu/custom; newly-added locations POST through /api/admin/menu/custom and inherit the canonical modifier structure with the first location's prices as the starting deltas. Scales cleanly to 20+ locations because the list view never enumerates them inline — the detail page is the only surface that does, and both per-location tables (base price + modifier pricing) are scrollable.",
        },
        {
