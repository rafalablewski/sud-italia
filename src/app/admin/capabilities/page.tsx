    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: "/admin/kds",
          summary:
            "Demo / training tool with no separate page: flip kdsSimulatorEnabled in /admin/settings (owner-only toggle) and the Kitchen Display itself becomes the generator. While any KDS board is open, useKdsSimulator streams synthetic orders onto the live /admin/kds (and mobile) board at organic, randomized intervals (uneven bursts and lulls, ~4.5–14s apart, with occasional double arrivals) and auto-advances them New → In progress → Ready → Done on per-ticket jittered dwell timers (each phase 60–150% of base, derived deterministically from the order id), so tickets pop in and move through the board like a real rush rather than on a fixed metronome. Every ticket is unmistakably marked — a purple dashed frame + 'SIMULATION — not a real order' tag per ticket, plus a board-wide 'Simulation mode' banner. Orders are built ONLY from the truck's real menu via getMenuWithOverrides() (no made-up products) through createSimulatedOrder(), which fires the same order-created SSE event a real checkout does so the board lights up live. Every order carries simulated:true, so getOrders() filters them out of EVERY non-KDS read by default: the dashboard, Orders list, /kitchen station + expo screens, fleet/floor-ops views and every report / CRM / analytics stay clean, and sims never trigger stock decrement, customer rollups or outbox SMS/email. Only the Kitchen Display boards opt in (?includeSimulated=1). Advancing a sim on the board (cook taps Start prep / Mark ready / Bump) is routed through updateOrderStatus, which short-circuits all side effects for simulated:true. Targets the truck in the top-bar location selector; the generator endpoint self-caps active sims (40) and bounds reads to the last 24h. Turning the toggle off purges every simulated ticket.",
          caveats:
            "Off by default — flip it on at /admin/settings (owner). Spawn/advance API rejected when the toggle is off (purge always allowed for cleanup); the generator endpoint is kitchen+ so it runs for whoever is at the pass. The generator runs client-side from an open KDS board, so sims stream while a board is open and pause (still visible) when none is — turning the toggle off clears them. Sims drive the orders-based board but do NOT fire per-station kds_tickets rows (that table has no cascade on order delete, so firing them would orphan rows on purge), so bump-time P95 stays '—' during a pure-sim demo.",
        },
        {
