    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: "/admin/kds",
          summary:
            "Demo / training tool with no separate page: flip kdsSimulatorEnabled in /admin/settings (owner-only toggle) and the Kitchen Display shows manual Add 1 / Add 5 / Purge all controls in the sandbox banner (desktop + mobile). There is NO auto-spawn or auto-advance — the operator adds a controlled batch of tickets, then works each one through the board with the normal Start prep / Mark ready / Bump buttons, so the demo is fully under their control rather than a random trickle. Every ticket is unmistakably marked — a purple dashed frame + 'SIMULATION — not a real order' tag per ticket, plus the board-wide banner. Orders are built ONLY from the truck's real menu via getMenuWithOverrides() (no made-up products) through createSimulatedOrder(), which fires the same order-created SSE event a real checkout does so the board lights up live (the board also calls refresh() after each Add/Purge for instant feedback). Every order carries simulated:true, so getOrders() filters them out of EVERY non-KDS read by default: the dashboard, Orders list, /kitchen station + expo screens, fleet/floor-ops views and every report / CRM / analytics stay clean, and sims never trigger stock decrement, customer rollups or outbox SMS/email. Only the Kitchen Display boards opt in (?includeSimulated=1). Advancing a sim on the board is routed through updateOrderStatus, which short-circuits all side effects for simulated:true. Targets the truck in the top-bar location selector; the endpoint self-caps active sims (40) and bounds reads to the last 24h. Turning the toggle off purges every simulated ticket.",
          caveats:
