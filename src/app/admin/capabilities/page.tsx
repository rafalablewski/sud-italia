    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          status: "live",
          href: "/admin/upsell",
          summary: "Decoy + anchor + default-pushed combos surfaced in the cart drawer above the per-item chips. Two pricing modes: **Fixed** (lunch ladder — solo eating, locked composition + price) and **Dynamic** (family ladder — pizzas/pastas scale with cart, add-ons stay static, price = `(mains-à-la-carte + cheapest-add-ons) × (1 - discountPercent/100)`). Lunch ladder is hour-gated (default 11:00–14:00); family ladder is quantity-gated (default ≥2 pizza+pasta items, one-line nudge at 1 short). Family tier ships at 20% off, Family Feast 28% (anchor), Feast Deluxe 24% with `minMains: 3`. Tapping a tier locks the cart subtotal to the computed price via `appliedBundleId`; checkout sends one Stripe line at the same price with composition itemized. `computeBundlePrice` runs identically on client, server, and Stripe-session creation so the displayed total always matches the charge. Both the bundle ladder (tiers, slot composition, pricing mode, discount %, min/max mains, default/anchor/decoy flags) and availability rules are CRUD-editable in /admin/upsell → Bundle ladders / Bundle rules. Validation rejects dynamic bundles whose composition slot category overlaps a mainCategory (would double-count), and fixed bundles with `refPrice < price` (would show negative savings). DEFAULT_BUNDLES + DEFAULT_BUNDLE_RULES in src/lib/bundles.ts fall back when no admin override is saved. Audit §3.2.",
        },
        {
