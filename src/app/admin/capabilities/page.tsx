    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: "/admin/menu",
          summary:
            "Every product on /admin/menu exposes the same editable surface regardless of storage origin: name, SKU (operator inventory code), category, price, food cost (locked when a recipe is attached), description, tags, availability, delivery-only flag, packaging cost override, and modifier groups. Seed-backed items route edits through the MenuOverride pipeline (category/tags/sku join the existing override fields with `null = clear back to seed` semantics); admin-created rows route through /api/admin/menu/custom. The legacy 'Custom' badge was retired — operators no longer need to think about storage backend. The 'Add item' dialog lets managers pick one, several, or all active locations; each target gets a separate row with a location-prefixed id (krk-…, war-…). Custom rows still surface a trash icon for hard-delete; seed-backed items revert via the bulk 'Reset overrides' action.",
        },
        {
