    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          status: "live",
          href: "/admin/upsell",
          summary: "Logistic-regression cross-sell ranker trained on REAL orders (audit elite-qsr §1) — no hardcoded weights. src/lib/ml-upsell.ts builds a leakage-controlled training set (one example per anchor-order × attach-candidate; per-customer attach features use only prior orders), fits weights by gradient descent over 7 learned features (personal attach rate, has-ordered, global attach rate, category×hour attach, item margin, tenure, new-customer flag), and ranks candidates by expected contribution (P(attach) × margin). POST /api/admin/ml-upsell/train pulls getOrders for the window + getMenuWithOverrides per location and persists per-location models (ml-upsell-models.json); GET returns model status (trainedAt, sampleCount, positiveRate, logLoss). Cold-start locations (<200 examples) are skipped → rules ranker stays in use. Inference: the cart POSTs to /api/customer/upsell-rank, which deterministically phone-buckets the customer into the ML arm when their bucket falls under the location's mlUpsellRolloutPct (the same hash is reproducible from any order's phone, so ML-vs-rules arms can be compared retroactively without storing assignments) and returns the model-ranked candidate ids; the cart orders the .v8-cart-pairs rail by them, falling back to the rules getCartSuggestions for the control arm / cold-start / untrained locations so cross-sell can never break. Operated from /admin/upsell → Cross-sell intelligence (MLUpsellPanel): model status, Train now, and the rollout-% slider. 12 unit tests cover separation, no-leakage construction, and recovery of a synthetic per-hour preference.",
        },
        {
