    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          summary:
            "Demo / training tool, gated like the finance simulation: flip kdsSimulatorEnabled in /admin/settings and an Order simulator tab appears under Kitchen Display in the nav. The page (manager+) streams synthetic orders onto a self-contained Kitchen Display board INSIDE the tab — Start/Stop, spawn-rate (trickle / steady / lunch-rush), Spawn one / Spawn 5, KPIs, and a 3-column New → In progress → Ready board with live age clocks — with auto-advance walking each ticket through the columns to Done on dwell timers. Orders are built ONLY from the truck's real menu via getMenuWithOverrides() — no made-up products — and persisted through createSimulatedOrder() and carry a '(sim)' customer-name tag. Every order carries simulated:true, so getOrders() filters them out of EVERY other read by default: they never appear on the live /admin/kds board, the /kitchen station + expo screens, the dashboard, the Orders list, the fleet/floor-ops views or any report / CRM / analytics, and they never trigger stock decrement, customer rollups or outbox SMS/email. The simulator tab is the only surface that opts in (its own /api/admin/kds-simulator route). Targets the truck in the top-bar location selector; the endpoint self-caps active sims and bounds its reads to the last 24h. Purge in one click; turning the toggle off hides the tab and purges leftovers (purge stays allowed even when disabled).",
          caveats:
            "Off by default — flip it on at /admin/settings. Nav tab hidden + spawn/advance API rejected when off (purge still allowed for cleanup). The generator runs client-side from the simulator page (polling its own route), so it streams while that tab is open. Simulated tickets are fully contained in the simulator tab — by design they do NOT fire order events or appear on the real KDS / kitchen screens, and they don't fire per-station kds_tickets rows (that table has no cascade on order delete, so firing them would orphan rows on purge).",
        },
        {
