      ...
      items: [
        {
          ...
          href: "/admin/agent-hq",
          summary:
            "The Boardroom rebuilt as a full agent-operations console with six sections — Command center (the agent roster + an org/reporting chart over live traffic-light KPIs: today's sales, food cost %, labour %, prime cost, avg ticket, satisfaction, refund rate, SSSG, each with a 5-section ⓘ explainer, plus today's spend vs cap per agent), Scorecards (per-agent KPI cards), Work (cross-agent activity feed + schedules + convene-the-board), Approvals (the human-in-the-loop queue of gated actions with Action / Mark done / Dismiss that transitions decision status), Inbox (chat any agent + an escalations panel of agents asking for the human), and Reports (meeting transcripts + decisions + spend). Nine agents seed it: four C-suite executives (CEO/COO/CFO/CMO) plus five specialist advisors (Frontend, Database, UX/UI, Market, Security). EVERY agent is editable end-to-end — name, role/title, status, reporting line, model, effort (thinking depth/token spend), authority level (observer/advisor/operator), runtime managed memory, mandate, responsibilities, KPIs, guardrails & ethics, escalation threshold, tone, collaborators, tool allowlist (the full role-gated registry, with ·writes badges on mutating tools), spend controls (daily + per-run caps) and schedule — and the editor renders the LIVE SYSTEM PROMPT generated from those fields, which is exactly what the agent runs on; a Reset-to-defaults drops the override and the Timeline tab shows the agent's run/edit history. Edits drive runtime: the agent loop + meetings + scheduled runs read the resolved config (generated prompt, tools ∩ role ∩ authority, model, effort, spend caps, status) so a rename / pause / model swap / guardrail change takes effect on the next turn. Every agent carries an escalate_to_admin lever that lands a real Inbox item + timeline entry. Overrides persist as a per-agent patch over the seed defaults (agent-configs.json), edits write a before/after audit row, and a per-agent timeline (agent-events.json) logs runs/edits/escalations/approvals with spend. Two crons: the daily boardroom briefing (whole board) and per-agent scheduled self-reviews by cadence (/api/admin/cron/agent-runs?cadence=daily|weekly). The active model (Claude or Gemini) is switchable inline. KPIs + configs render even without a key; chat + meetings degrade to 'needs-config'. Shares the gateway, tool registry, conversation store, and daily budget with the Ops Agent.",
          caveats:
            "Food cost %, refund rate, and SSSG are chain-wide (computed across all locations); today's sales, labour %, and satisfaction honour the location switcher. Meeting decisions are advisory until an operator approves the proposed action; 'Mark done'/'Dismiss' transition the decision so it leaves the queue. Per-agent scheduled runs require the agent-runs cron to be wired into the dispatcher (the route + runner ship here). /admin/boardroom redirects to /admin/agent-hq.",
        },
        {
