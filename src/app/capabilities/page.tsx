    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          summary:
            "Sandbox monthly P&L bound to real-order actuals (orders/day, AOV, weighted COGS, delivery share, refund rate, median ticket time — all pulled from /api/admin/orders over a 90-day rolling window and applied with one click). Tune revenue inputs, labor mix (with volume-flex), fixed costs, the per-revenue constants (payment / refund / loyalty / CIT / D&A / interest), kitchen capacity (peak-hour throughput ceiling), and channel-split payment fees (cash / on-site card / Glovo / Wolt). Food cost + waste are derived from the dish recipes (menu-mix-weighted) and read-only on the five named scenarios — only the Custom scenario unlocks them for hand-typed what-ifs. The Scenarios picker (five archetypes + Custom) sits directly below Variable costs. A Premises card models the occupancy decision across three modes — Rent, Mortgage or Buy (cash) — (rent + deposit + service charge; or purchase price + down payment + mortgage rate/term; or an all-cash purchase — all with property tax + structural upkeep + building depreciation + appreciation on the owned modes): computePremises/applyPremises derive the rent line, mortgage interest, building depreciation, property costs and the upfront cash from it, so the choice flows all the way to payback + IRR. A ‘Premises ROI vs markets’ card then runs a real month-by-month simulation of the whole horizon (default 10 years) for each of the three scenarios — projectMonths composing seasonality, weather and compounding inflation (labour + fixed incl. rent at wage CPI, COGS at ingredient CPI, menu prices at a menu-inflation lever), so a rising-rent lease genuinely looks worse over time while a fixed mortgage is eroded by inflation — and scores each mode's annualised return (IRR of the full cash-flow stream incl. terminal building equity) against the S&P 500, Nasdaq-100 and a 5% bond, answering whether it's viable to run the unit or better to invest the capital. A collapsible ‘How this is calculated’ panel explains the method inline, and an ‘Alternatives — same capital, deployed differently’ set ranks seven equal-capital strategies by 10-yr total wealth: two pure-passive baselines (skip the restaurant — the whole cheque into a 10y bond or the S&P today) plus five rent-and-deploy plays (rent the unit + put the freed capital into the S&P / a bond / the Nasdaq-100 / a 60-40 portfolio / more rented units), each compared head-to-head against buying/mortgaging outright (computePremisesInvestment). 9 behavior levers, 5 weather/calendar levers, per-month seasonality overrides. Institutional-grade KPI suite: EBITDA, EBITDAR, cash-on-cash return, occupancy ratio, refund-adjusted net sales, contribution per labor hour (QSR target ≥150 zł/h), promo-adjusted AOV, peak orders/hour, median ticket time, true contribution margin, kitchen-capacity utilisation. Two 2-D heatmaps, scenario comparison, ±20% sensitivity, sensitivity tornado across all key drivers, 12-month operational projection, and a 24-month investor view with 4-month opening ramp surfacing NPV @ 10/15/20%, IRR, and cumulative-cash break-even. Break-even chart shows the current operating point vs ceiling at a glance. Master toggle in Settings → General. Defaults are Warsaw 2026 (gross × 1.22 ZUS narzut, 5-year truck depreciation). Zero writes to the business-costs ledger.",
        },
        {
        ...
        },
        {
          ...
          href: "/core/service/slots",
          summary:
            "Atomic increment (no overselling). Auto-close past slots via cron. Dine-in seating windows are open by default: ensureDineInSlots (src/lib/store.ts) materialises a slot every 15 min across the location's open hours for the whole floor (15-min step matches the 15-min table turnaround so a freed :45 seating is actually offered; capacity = live table count, deterministic dineInSlotId ids so it's idempotent and bookings attach across reloads), pulled via GET /api/admin/slots?ensureDineIn=1 by both Slots and Book. Operators don't build the grid — they close windows: each row has an Available↔Unavailable toggle (persists as active/draft, hidden from Book when closed), or Select mode marks several unavailable in one PUT (block an evening for a private event). Grid windows are never deleted (they'd just regenerate); only manual slots keep a delete affordance.",
        },
        {
