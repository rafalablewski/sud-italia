      ...
      items: [
        {
          ...
          href: "/admin/agent-hq",
          summary:
            "The Boardroom rebuilt as a full agent-operations console with six sections — Command center (the agent roster over live traffic-light KPIs: today's sales, food cost %, labour %, prime cost, avg ticket, satisfaction, refund rate, SSSG, each with a 5-section ⓘ explainer), Scorecards (per-agent KPI cards), Work (cross-agent activity feed + schedules + convene-the-board), Approvals (the human-in-the-loop queue of gated actions), Inbox (chat any agent), and Reports (meeting transcripts + decisions + spend). Nine agents seed it: four C-suite executives (CEO/COO/CFO/CMO) plus five specialist advisors (Frontend, Database, UX/UI, Market, Security). EVERY agent is editable end-to-end — name, role/title, status, reporting line, model, effort (thinking depth/token spend), authority level (observer/advisor/operator), runtime managed memory, mandate, responsibilities, KPIs, guardrails & ethics, escalation threshold, tone, collaborators, tool allowlist, spend controls (daily + per-run caps) and schedule — and the editor renders the LIVE SYSTEM PROMPT generated from those fields, which is exactly what the agent runs on. Edits drive runtime: the agent loop + meetings read the resolved config (generated prompt, tools ∩ role ∩ authority, model, effort, spend caps, status) so a rename / pause / model swap / guardrail change takes effect on the next turn. Overrides persist as a per-agent patch over the seed defaults (agent-configs.json) and a per-agent timeline (agent-events.json) logs runs/edits with spend. The active model (Claude or Gemini) is switchable inline; a daily cron auto-runs the briefing. KPIs + configs render even without a key; chat + meetings degrade to 'needs-config'. Shares the gateway, tool registry, conversation store, and daily budget with the Ops Agent.",
          caveats:
            "Food cost %, refund rate, and SSSG are chain-wide (computed across all locations); today's sales, labour %, and satisfaction honour the location switcher. Meeting decisions are advisory until an operator approves the proposed action. Per-agent schedules are persisted + displayed; the daily-cadence executives fire via the existing briefing cron (no per-agent cron yet). /admin/boardroom redirects to /admin/agent-hq.",
        },
        {
