...
export default async function CapabilitiesPage() {
  ...
  const groups: CapabilityGroup[] = [
    ...
    {
      id: "apps",
      title: "Installable apps — native iOS + web (PWA)",
      items: [
        {
          ...
          href: "/",
          summary:
            "The customer-facing app: order Neapolitan pizza/pasta and carry the Soci e amici loyalty card. Two install paths, one web codebase. (1) Native iOS app — a pure-UIKit WKWebView wrapper (native/ottaviano-ios, target Ottaviano) that renders the live storefront 1:1, so every screen IS the web UI and can never drift; App-Store shippable via the xcodegen project (Scripts/testflight.sh). (2) Installable web PWA on Android/desktop — the root layout advertises /manifest.json + the apple-web-app title \"Ottaviano\" + brand-red theme, portrait, own icon set (public/icons/ottaviano/*). The in-page Install button (src/components/pwa/InstallAppButton.tsx) fires the Chromium install or shows an iOS Add-to-Home-Screen how-to, and self-hides inside the native wrapper (UA carries a NativeWrapper token). No config — works in demo mode.",
        },
        {
          ...
          href: "/operator",
          summary:
            "The operator app for tablets/iPads: full Admin back-office + all of Core (POS, KDS, Orders, Guest, Service). Two install paths, one web codebase. (1) Native iOS app — a pure-UIKit WKWebView wrapper (native/ottaviano-ios, target OttavianoKDS) that opens /operator on the live host and renders the operator suite 1:1; dark/landscape chrome, distributed via Apple Business Manager (internal staff tool). Because it renders the web, the native KDS reflects the web app exactly — there is no second UI to keep in sync. (2) Installable web PWA for non-iOS operators — the admin, core, kitchen, operator (+ admin login/welcome) layouts advertise /ottaviano-kds.webmanifest + the apple-web-app title \"OttavianoKDS\" + a dark theme, landscape-first, own icon set (public/icons/kds/*). Both apps coexist on one home screen because each route subtree overrides the manifest + apple title (see src/lib/pwa.ts); one service worker (public/sw.js) serves both shells.",
        },
        {
          name: "Native iOS shell (WKWebView, no SwiftUI)",
          status: "live",
          href: "/operator",
          summary:
            "native/ottaviano-ios is a reproducible xcodegen project (project.yml → two app targets, Ottaviano + OttavianoKDS) whose entire UI is one shared pure-UIKit web shell (Sources/WebShell: AppDelegate + SceneDelegate + WebAppViewController + WebAppConfig). It hosts a single WKWebView that renders the live web app, so each app reflects the web 1:1 by construction — the deliberate replacement for the retired SwiftUI rebuild, which had to mirror the web by hand and could drift. The only per-app difference is data in each Apps/<App>/Info.plist (OTTWeb* keys: which URL to open, brand chrome); OTTAVIANO_WEB_BASE_URL overrides the host for staging/dev. The shell adds native polish over Safari: a persistent data store (session survives relaunch), the NativeWrapper UA token (so the web hides its install prompt), pull-to-refresh, swipe-back/forward, a slim progress bar, a branded offline-retry screen, and system handling of tel:/mailto:/_blank links. Because it's a web wrapper it needs no JSON API facade — it uses the same web routes + cookie auth as the browser. Built on a Mac/CI (this Linux container can't compile iOS); ships via Scripts/testflight.sh + .github/workflows/ios*.yml.",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: "/admin/customers",
          summary:
            "PENDING — re-build on the web before the customer iOS app submits. Self-serve delete/export used to live on the native customer app's More tab via DELETE/GET /api/v1/customer/account[/export]; that surface was retired with the /api/v1 facade when the native apps became WKWebView shells of the web. Apple Guideline 5.1.1(v) still requires in-app account deletion for any app with sign-in, so the customer-facing storefront needs a web account page that calls the existing deleteCustomerData / exportCustomerData helpers (the operator GDPR tool already uses them — src/app/api/admin/gdpr/delete) scoped to the cookie-identified customer. Not a blocker for OttavianoKDS (operator app → Apple Business Manager, no consumer sign-in deletion requirement).",
        },
      ],
    },
    {
