    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: "/admin/settings",
          summary:
            "Owner-only switch (Settings → Simulations) for rehearsing the full business before go-live. Like Sandbox it flips the ENTIRE app — admin AND storefront — onto an isolated namespace (`sim:` prefix via the same src/lib/store.ts resolveKey + getDomainDb path) so real data is physically untouched. First enable seeds the same full CORE picture as Sandbox but DEEPER — seedSimulation() (src/lib/sandbox/seed.ts → the shared seedDataset('simulation') body) lays down ≈90 days of trading at a few orders/day per location (vs Sandbox's compact ~30-day demo), spread across the service window (11:00–22:00) so Reports, Cohort/LTV, SSSG, Dayparts and Hourly-throughput all have real signal — alongside tables, slots, staff+schedule, cash reconciling to revenue, bookings, suppliers/POs, waste/HACCP and feedback/surveys, so every operational surface (POS/KDS/Orders/Service/Guest) is testable the instant you switch on. Orders land via bulkAppendOrders (one locked write per location, not createOrder's per-insert O(N) blob rewrite) so even the deep dataset seeds in well under the serverless budget on Neon; the seeder then fires KDS + rebuilds CRM rollups once. From there you keep pushing your own test orders, waste write-offs, costs and customers by hand to dry-run the whole flow. Edits persist across off→on (rows are kept so you can switch back on and continue); a 'Reset & re-seed' button wipes + re-seeds a clean dry-run, and a 'Wipe to empty' button clears the namespace for pure hand-entry. Shared/real keys (menu, recipes, ingredients, auth, locations, config) stay live, identical to Sandbox. AI agents keep working: the interactive Ops/Boardroom agents read the sim namespace automatically (their tools route through the store), and the cron dispatcher (src/app/api/admin/cron/dispatch) runs the ANALYSIS/AI jobs in simulation (boardroom daily briefing, customer-segments rebuild, daily summary, labor-efficiency, inventory-variance, lapsed-detect, weather-staffing, par-PO drafts) so they learn from and report on the dry-run data for the operator to check — while every real-world job stays paused (outbox/SMS/email drain, WhatsApp sends, corporate reminders + invoices, retention-trim of real audit tables, DB backup; flagged `realWorld` in the dispatcher). Customer-facing sends + Stripe charges initiated by agents are independently suppressed by the comms/checkout `isSandboxActive()` guards (true in both modes). An indigo banner shows across admin + storefront so nobody mistakes the dry-run for live trading. Mutually exclusive with Sandbox mode — enabling either forces the other off (one namespace prefix is live at a time). Endpoints: POST /api/admin/simulation-mode {enabled} | {action:'reset'} | {action:'wipe'}. Distinct from the per-record `simulated` flag, the finance Calculator (/admin/simulation), and Sandbox mode.",
          caveats:
