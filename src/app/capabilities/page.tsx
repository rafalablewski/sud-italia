  ...
  const groups: CapabilityGroup[] = [
    {
      ...
      items: [
        ...
        {
          name: "Team tasks & announcements (comms)",
          status: "live",
          href: "/admin/comms/tasks",
          summary: "Internal comms board, split into two separate Overview surfaces: TASKS (/admin/comms/tasks) and ANNOUNCEMENTS (/admin/comms/announcements) — each its own nav entry/route, no shared tab. An owner — or anyone granted comms.manage in the Permission Matrix — assigns to-do TASKS to a specific teammate or a whole role (optionally scoped to a location; role-targeting fans out to one task row per person, each with its own done-state), and posts ANNOUNCEMENTS (Title subject + textarea body) to everyone / chosen roles / chosen locations / named people (pinnable, edit-in-place + delete). Each teammate sees their own tasks + targeted announcements on their role portal (Manager/Franchisee, via PortalInbox): announcements lead as a Gmail-style NOTIFICATION inbox with three mailbox tabs — INBOX / ARCHIVED / DELETED — and per-row actions Mark read · Archive · Delete (Restore from Archived/Deleted). The Inbox shows only the 3 most-recent UNREAD rows with a 'Load more' beneath; 'Your to-do list' sits below — and it isn't read-only: a quick-add box (title + priority + optional due date) lets ANY teammate add an item to their OWN list (POST /api/admin/my-tasks stamps the session user as both assignee + creator, so it can never assign onto someone else), self-added items (createdBy === assigneeId) show 'added by you' and can be removed outright (DELETE /api/admin/my-tasks, restricted to items you created — manager-assigned tasks only offer Done). Mailbox state is per-recipient (archivedBy/deletedBy on the announcement, deleted wins over archived) so one person archiving never hides it for others. Every interaction — open/read, archive, delete, restore — PUTs {id,action} to /api/admin/my-announcements and is written to the central AUDIT LOG as notification.{read,archive,delete,restore} (entityType=announcement) so an owner can review who did what, when. The unread + open-to-do count also surfaces in CommsBell (src/components/portal/CommsBell.tsx) — an inbox-icon button rendered in the shell topbar AND on both portal headers, deliberately separate from the operational alerts bell beside it; its glance dropdown links to the portal inbox. Receiving needs NO permission (personal feeds on /api/admin/my-tasks + /api/admin/my-announcements, any authed user, identity = session never a query param); only the management board is gated (comms.view to open, comms.manage to assign/post/edit/delete — owner-default, grantable). Persisted via the store (readJSON/writeJSON → Postgres KV + filesystem); recipient matching + mailbox-state helper + types in src/lib/comms.ts; writes audited (tasks.assign / announcements.post / notification.*). No mock data — empty until someone assigns/posts.",
        },
        {
