  ...
  const groups: CapabilityGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          status: "live",
          href: "/admin/settings",
          summary: "Swap the entire look of any surface — Business (storefront), Admin, or Core — to a totally distinct theme (its own classes, selectors & tokens), DB-global so one choice repaints for every visitor. Pick a skin per surface at /admin/settings → Themes → Active skins (saves instantly per Rule #7, owner-gated + audited via PUT /api/admin/themes). The registry (src/lib/theme-skins.ts) lists skins per surface; the shipped theme is the 'default' skin and each alternate is a self-contained stylesheet scoped under [data-skin=\"<id>\"] (homepage/skins/notte.css, admin-v3/skins/blueprint.css, core/skins/solare.css). Admin + Core server-render data-skin onto the surface root (already dynamic → no flash); the storefront stays static and applies its skin via /api/settings/public (homepageSkin) + a pre-paint boot script + HomepageSkinSync on <body> (reaches Rule-#4 portal overlays, cleans up on nav). Ships with the design-system docs (docs/design-system/{README,*/skins}.md) + themes.manifest.json per Rule #11. First pass ships one example alternate skin per surface: Notte (candle-lit night storefront), Blueprint (navy/cyan admin console), Solare (warm-daylight Core).",
        },
        {
