  ...
  const groups: CapabilityGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          name: "Native apps — React Native (Expo) rebuild",
          status: "live",
          href: "/api/v1/openapi.json",
          summary:
            "The native iOS rebuild of both apps in React Native + Expo + expo-router, living at native/ottaviano-rn (one TypeScript codebase shared with the web, buildable via GitHub Actions/EAS or Xcode). Replaces the retired SwiftUI seed (native/ottaviano-ios, deleted). Consumes the same /api/v1 facade as the spec'd Swift seed did. Ottaviano (customer, warm parchment skin): full order path — browse the live menu (GET /menu) → cart → server-priced POST /api/v1/orders (guest or phone-OTP customer) → live SSE order tracker (/customer/orders/:id/stream) → Rewards loyalty card + order history + account delete/export. OttavianoKDS (operator, always-dark KDS skin): the full 54-surface shell with a role-filtered slide-in drawer mirroring the web admin rail (the OPERATOR_NAV generated from src/admin-v3/nav.config.ts + src/core/routes.ts), the Kitchen Display at depth (live SSE lanes Floor/Chef/Fleet, bump via PATCH /api/v1/orders/:id, recall, 86 via PATCH /api/v1/admin/menu, all-day, station filter — 1:1 with src/core/kds/CoreKds.tsx), the live Orders board (/orders/stream) and Dashboard/Reports (/admin/summary), and every other surface data-backed off its /api/v1/admin/* endpoint via a generic DataSurface (real rows, no mock data — Rule #1) or an honest parity scaffold (SOC 2 + Capabilities, the content pages with no data source). The two app skins + the operator IA are GENERATED from the web source (scripts/gen-native-tokens.ts → src/theme/tokens.generated.ts; scripts/gen-native-nav.ts → src/nav/operatorNav.generated.ts) so they can't drift — the native parity gate (npm run check:native) is repointed at native/ottaviano-rn. Set EXPO_PUBLIC_API_BASE_URL (or app.json extra.apiBaseUrl) to the /api/v1 origin — the only host reference, so the Vercel exit needs no client release. CUSTOMER PARITY (web storefront 1:1): the Order tab is the full menu surface (search, All + per-category tabs, live open-now pill off location hours, an operator-set speed-guarantee banner + combo previews) whose cards open a real item-detail sheet — allergens, bilingual nutrition readout, and a modifier picker (radio/checkbox required-gated off MenuItem.modifierGroups) with a live re-quoting paybar; the Cart is the whole checkout (modifier-keyed lines + per-line notes, the cross-sell pairing rail off POST /api/v1/upsell, a combo banner that subtracts the real saving per Rule #8, a tip picker, a fulfilment toggle revealing a delivery address / dine-in party stepper, the loyalty earn preview + min-order gate, server-priced POST /orders carrying selectedModifiers/tip/address/party); the live tracker adds an ETA card, fulfilment chip, points-earned and share; Orders splits Active⇄Past with one-tap Reorder; Rewards is the tier card with live progress, the full tier roadmap, the rewards catalogue (affordable vs locked) and referral terms — all off the new GET /api/v1/settings/public. Pending before an App Store submission: Stripe PaymentSheet wiring (endpoint live) and a green CI iOS build (needs EXPO_TOKEN + EAS-managed Apple signing).",
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          name: "Native storefront facade — public settings + cross-sell",
          status: "live",
          href: "/api/v1/settings/public",
          summary:
            "Two additive public reads that bring the customer app to web-storefront parity (Rule #6, no auth). GET /api/v1/settings/public is the single programme-config read: the loyalty tier ladder (label/threshold/multiplier/perks), the active rewards catalogue, the give-get referral mechanics, the combo-deal ladder (DEFAULT_COMBO_DEALS — the same source the web menu reads), the speed-guarantee banner, and the delivery-fee / free-delivery-threshold / min-order / tip-preset money config — every value mapped from the live getLoyaltySettings()/getSettings() store the operator writes to (Rule #1), so a loyalty/combo edit in /core/guest/loyalty or /admin/settings lands in the app with no App Store release. POST /api/v1/upsell runs the storefront getCartSuggestions engine over the live menu — the customer twin of the staff /admin/pos/suggestions panel — returning the four-slot espresso → tiramisù → garlic-bread → limonata 'complete your meal' rail for the cart + post-order surfaces (CLAUDE upsell rule). Both are Zod-validated, OpenAPI-published (PublicSettings / UpsellSuggestion / ComboDeal schemas) and contract-tested (tests/api-v1-storefront.test.ts). Works in demo — no config.",
        },
        {
