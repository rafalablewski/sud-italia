  ...
  const groups: CapabilityGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          name: "Sandbox mode (whole-business demo)",
          status: "live",
          href: "/admin/settings",
          summary:
            "Owner-only switch (Settings → Simulations) that flips the ENTIRE app — admin AND the customer storefront — onto an isolated demo dataset, so you can explore/train/screenshot a fully-populated business without touching real data. Isolation is a storage namespace: when on, every non-shared store key is transparently prefixed `sandbox:` (src/lib/store.ts resolveKey + getDomainDb, which routes the normalized Postgres domains through their kv fallback — no migrations), and a 2.5s-cached flag off the shared settings blob drives it. Shared/real (never namespaced): the menu, menu-overrides (prices/86s), recipes, ingredients, admin auth/users, locations, and config. Sandboxed (demo): orders, customers, KDS, stock, suppliers/POs, staff, schedule, time-punches, cash, waste, HACCP/temp, feedback, surveys, loyalty, notifications, tasks, audit. First enable seeds rich, internally-consistent data via seedSandbox() (src/lib/sandbox/seed.ts — orders drive KDS+CRM+analytics+loyalty, schedule references seeded staff, cash reconciles to cash-order revenue, and AI agent-event spend is seeded across today + yesterday so the Morning Brief AI-spend module + Agent HQ aren't empty); edits persist across off→on; a 'Reset sandbox' wipes + re-seeds. While on, all external side-effects are suppressed (Stripe checkout, SMS/email outbox, WhatsApp sends, admin push, every cron job) and a fixed banner shows across admin + storefront. Toggle off restores real instantly. Mutually exclusive with Simulation mode. Endpoints: POST /api/admin/sandbox {enabled} | {action:'reset'}. Distinct from the per-record `simulated` flag and the Calculator.",
          caveats:
            "Whole-business: while on, the live storefront serves demo data too — don't enable during real trading (it's a deliberate 'operations paused' state). Shared domains (menu/recipes/ingredients) are genuinely shared, so editing them in sandbox affects real — they're meant to stay identical. Cross-instance the flag is eventually-consistent within ~2.5s of toggling. KDS ticket analytics (bump-time P95) read a DB-only ledger and stay empty in sandbox.",
        },
        {
          ...
          href: "/admin/settings",
          summary:
            "Owner-only switch (Settings → Simulations) for rehearsing the full business before go-live. Like Sandbox it flips the ENTIRE app — admin AND storefront — onto an isolated namespace (`sim:` prefix via the same src/lib/store.ts resolveKey + getDomainDb path) so real data is physically untouched, but UNLIKE Sandbox it never seeds: the dataset starts COMPLETELY EMPTY and you fill it by hand — pushing your own test orders, waste write-offs, costs, customers, bookings, cash sessions — exactly as you would once trading, to dry-run the whole flow. Toggling off hides every test row instantly (rows are kept so you can switch back on and continue); a 'Wipe simulation data' button clears the namespace for a fresh start. Shared/real keys (menu, recipes, ingredients, auth, locations, config) stay live, identical to Sandbox. AI agents keep working: the interactive Ops/Boardroom agents read the sim namespace automatically (their tools route through the store), and the cron dispatcher (src/app/api/admin/cron/dispatch) runs the ANALYSIS/AI jobs in simulation (boardroom daily briefing, customer-segments rebuild, daily summary, labor-efficiency, inventory-variance, lapsed-detect, weather-staffing, par-PO drafts) so they learn from and report on the dry-run data for the operator to check — while every real-world job stays paused (outbox/SMS/email drain, WhatsApp sends, corporate reminders + invoices, retention-trim of real audit tables, DB backup; flagged `realWorld` in the dispatcher). Customer-facing sends + Stripe charges initiated by agents are independently suppressed by the comms/checkout `isSandboxActive()` guards (true in both modes). An indigo banner shows across admin + storefront so nobody mistakes the dry-run for live trading. Mutually exclusive with Sandbox mode — enabling either forces the other off (one namespace prefix is live at a time). Endpoints: POST /api/admin/simulation-mode {enabled} | {action:'wipe'}. Distinct from the per-record `simulated` flag, the finance Calculator (/admin/simulation), and Sandbox mode.",
          caveats:
            "Whole-business: while on, the live storefront serves your test data too — don't enable during real trading. The surface is empty until you add data — that's intentional, not a bug. Shared domains (menu/recipes/ingredients) are genuinely shared with real, so editing them here affects real. AI analysis jobs run on the sim data, so they DO consume the daily AI budget — keep an eye on Agent HQ spend during a long dry-run. (Sandbox mode, by contrast, pauses every cron including AI.) Cross-instance the flag is eventually-consistent within ~2.5s of toggling. KDS bump-time P95 analytics read a DB-only ledger and stay empty here, same as Sandbox.",
        },
        {
