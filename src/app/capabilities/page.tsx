  ...
  const groups: CapabilityGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          status: "live",
          href: "/admin/comms",
          summary: "Internal comms board at /admin/comms (Overview → Tasks & announcements). An owner — or anyone granted comms.manage in the Permission Matrix — assigns to-do TASKS to a specific teammate or a whole role (optionally scoped to a location; role-targeting fans out to one task row per person, each with its own done-state), and posts ANNOUNCEMENTS to everyone / chosen roles / chosen locations / named people (pinnable). Each teammate sees their own tasks + targeted announcements on their role portal (Manager/Franchisee 'Your to-do list' + 'Announcements', via PortalInbox) and ticks tasks done / marks announcements read. Receiving needs NO permission (personal feeds on /api/admin/my-tasks + /api/admin/my-announcements, any authed user, identity = session never a query param); only the management board is gated (comms.view to open, comms.manage to assign/post/delete — owner-default, grantable). Persisted via the store (readJSON/writeJSON → Postgres KV + filesystem); recipient matching + types in src/lib/comms.ts; writes audited (tasks.assign / announcements.post). No mock data — empty until someone assigns/posts.",
        },
        {
