  ...
  const groups: CapabilityGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          summary:
            "/core/service/tables (src/core/service/CoreTables.tsx) is the table PLAN, not the live room: it does one job — manage zones, tables and seats. /core and /core/service both land here and the Lens Rail's Service lens is 'Tables'. Tables are grouped by zone with a zone filter; a stat strip reads live off the catalogue (tables · seats · zones · available · out-of-service · accessible — Rule #1). Tapping a tile (or its ⋯) opens the table editor (CoreDialog, portaled per Rule #4): number/label, seats, zone, status (available/reserved/out-of-service; a table already seated by ops keeps that value so an edit can't free the party), accessibility features (accessible/high-chair/step-free) and a service note — all persisted to the shared per-location catalogue via /api/admin/floor/tables (Add/Save/Delete, manager+). There is deliberately NO seating, order lookup or live-occupancy tooling here — that operational flow lives in Book's Floor lens (seat parties, open checks) and POS. Reads/writes the same FloorTable rows every other surface shares, so a table added here shows up in the POS picker and Book instantly.",
        },
        {
