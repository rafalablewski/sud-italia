  ...
  const groups: CapabilityGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          summary: "Internal comms board, split into two separate Overview surfaces: TASKS (/admin/comms/tasks) and ANNOUNCEMENTS (/admin/comms/announcements) — each its own nav entry/route, no shared tab. An owner — or anyone granted comms.manage in the Permission Matrix — assigns to-do TASKS to a specific teammate or a whole role (optionally scoped to a location; role-targeting fans out to one task row per person, each with its own done-state), and posts ANNOUNCEMENTS (Title subject + textarea body) to everyone / chosen roles / chosen locations / named people (pinnable, edit-in-place + delete). Each teammate sees their own tasks + targeted announcements on their role portal (Manager/Franchisee, via PortalInbox): announcements lead the portal as a Gmail-style NOTIFICATION inbox (sender avatar + subject + snippet + timestamp, unread bold with a brand dot, pinned flagged; tap a row to open the full message and mark it read), with 'Your to-do list' beneath. Receiving needs NO permission (personal feeds on /api/admin/my-tasks + /api/admin/my-announcements, any authed user, identity = session never a query param); only the management board is gated (comms.view to open, comms.manage to assign/post/edit/delete — owner-default, grantable). Persisted via the store (readJSON/writeJSON → Postgres KV + filesystem); recipient matching + types in src/lib/comms.ts; writes audited (tasks.assign / announcements.post). No mock data — empty until someone assigns/posts.",
        },
        {
