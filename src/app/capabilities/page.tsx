      ...
      items: [
        {
          ...
          status: "live",
          href: "/admin-v3",
          summary: "Ground-up, density-first rebuild of the admin back-office (isolated under themes/admin-v3 + src/admin-v3 so v2 can be deleted at parity). Live at /admin-v3: the owner-gated Operator Terminal cockpit — revenue→daily-goal hero (real configurable goal via /api/admin/ops-goals, GET/PUT, chain + per-location), live KDS-fleet tiles (cooking/ready/late), the levers that move the goal, a ranked 'what moves it most', kitchen pace + truck status, order-flow, and a live feed/needs-you-now — all wired to real analytics/insights/kds-fleet/labour/orders/notifications endpoints, refetching every 30s. Orders (/admin-v3/orders) is live — a real-time Kanban + table over the SSE order stream with a detail dialog and pipeline status mutations (PUT /api/admin/orders), staff+. Inventory (/admin-v3/inventory) is live — stock table with value/low-out/7d-waste KPIs, status chips, a movements view, and an edit dialog that writes par/reorder/on-hand (PUT /api/admin/stock) and logs receive/waste/adjust movements (POST /api/admin/stock-movements), aggregating across trucks when scope=all. Menu (/admin-v3/menu) is live at full v2 parity — the chain-wide product board (one row per dish deduped by base slug per rule #10; diverging prices shown as a range + 'varies' badge) with multi-select + a sticky bulk toolbar (mark available / 86 / bulk-edit / clone-to-site / reset overrides / delete via POST /api/admin/menu/bulk), a Show-hidden toggle for soft-deleted seed rows, Add-item creating a chain-wide custom SKU on every site (POST /api/admin/menu/custom), and an edit dialog covering product metadata (name/description/category/tags/menu-role), service (delivery-only/packaging), a modifier-group editor, regulatory disclosures (halal/Nutri-Grade/pork/alcohol/allergens) and per-site price/cost/availability/SKU — written via PUT /api/admin/menu, plus per-dish reset/delete. Recipes (/admin-v3/recipes) is live at full v2 parity — a two-tab page: Recipes board (one recipe per dish keyed by base slug per rule #10; food-cost / cost-% / kcal) whose editor shows per-portion KPIs, a cost-breakdown bar, live per-portion macros, missing-kcal/no-distributor line flags, prep time + notes and saves chain-wide via POST /api/admin/recipes (wasteFactor now stored correctly as the 1+waste% multiplier, fixing an under-costing bug); and an Ingredients catalog with add/edit/delete (/api/admin/ingredients) plus a per-ingredient distributor-offerings manager (/api/admin/ingredient-products: add/edit/delete, supplier+SKU+cost+per-unit macros, make-active star via PATCH). Suppliers are read for the picker. The Operations logs are live too: HACCP (/admin-v3/haccp, temperature checks with live verdict), Waste (/admin-v3/waste, reason-coded write-offs + cost), and Shift handover (/admin-v3/handover, cash-variance + checks sign-off) — each a per-location record form + log over its real endpoint. Suppliers (/admin-v3/suppliers, chain-wide CRUD directory) and Purchase orders (/admin-v3/purchase-orders, per-location restock POs with a create dialog + draft→sent→received status flow where receiving auto-credits stock) round out Inventory. People is live: Staff (/admin-v3/staff — directory with clock in/out via /api/admin/time-punches + add/edit/delete) and Schedule (/admin-v3/schedule — this week's shifts grouped by day with add/edit/delete via /api/admin/shifts). Customers (/admin-v3/customers — phone-based directory with repeat/CLV KPIs + search, derived from real orders) and Feedback (/admin-v3/feedback — guest-review board with status flow + AI sentiment analyze) are live. Corporate (/admin-v3/corporate — B2B wallet accounts with bonus/billing editor) and Pulse surveys (/admin-v3/surveys — NPS pulse + survey active toggles) complete the Customers section. Finance is underway: Reports (/admin-v3/reports — range presets, revenue/profit/margin/tips KPIs, category bars, top items, JPK export) and Business costs (/admin-v3/business-costs — expense register with monthly/annualised/payroll KPIs + add/edit/delete) are live; Cash (/admin-v3/cash — till session lifecycle: open float, record sale/drop/payout, expected-drawer, close with counted-cash → variance, session history) is live. The Calculator (AdminSimulation) is the remaining Finance surface; Growth is underway: Scheduled bundles (/admin-v3/scheduled-bundles — standing pre-order approval board) and Truck ops (/admin-v3/truck — events + routes CRUD with a stops editor) are live; Campaigns (/admin-v3/growth — loyalty levers: referral + reward/challenge/seasonal toggles), Cross-sell (/admin-v3/crosssell — full v2 parity: four tabs over the per-location selling config (PUT /api/admin/upsell, full config round-tripped) — Cart pairings (Coffee/Dessert/Side/Drink slots), Combo deals CRUD, Time-of-day windows CRUD, and Menu badges multi-selects with menuRole-intrinsic auto-locks; the Combo deals tab is a Menu/Recipes-style workbench — a reactive KPI rail (active combos / avg discount / item-gated / windows-live-now, with five-section ⓘ explainers), search + Board⇄Table toggle, and a two-column drill-in editor with a live customer-nudge preview + a real-price worked złoty example; the Time-of-day editor gained a live cart-banner preview + showing-now indicator) and Upsell (/admin-v3/upsell — full v2 parity: Bundles tab with a full bundle-ladder editor (composition slots, fixed/dynamic pricing, anchor/decoy/default flags, loyalty/channel/members/scarcity/active-days gating), a bundle-rules card, an A/B experiment editor (variants/weights/per-bundle overrides/metric/control/start-stop/promote-winner) and an ML-ranker panel (rollout slider, Train-now via POST /api/admin/ml-upsell, model status, live ML-vs-rules comparison via /api/admin/ml-upsell/compare); plus a read-only Item-modifiers inventory — all round-tripped through PUT /api/admin/upsell; the Bundles tab is now a Menu/Recipes-style workbench wired to real 30-day /api/admin/bundle-analytics — a reactive KPI rail with five-section ⓘ explainers (active bundles / penetration / bundle AOV / 30d revenue / avg effective discount), search + Board⇄Table toggle, a meal-period-grouped board with per-bundle live 30d stats, and a two-column drill-in editor with a live bundle-card preview + this bundle's real 30d performance grid) complete the Growth section. Intelligence is underway: Multi-location (/admin-v3/locations — cross-site comparison), Menu engineering (/admin-v3/menu-engineering — star/puzzle/plowhorse/dog classification), and Expansion (/admin-v3/expansion — new-site readiness checklists) are live; Manage locations (/admin-v3/locations/manage — site CRUD with hours editor + re-seed) and Insights (/admin-v3/ai — AI demand forecast + chatbot-FAQ manager) complete Intelligence. System is underway: Audit log (/admin-v3/audit-log), SOC 2 controls (/admin-v3/soc2, real buildSoc2Register introspection), Currency + Languages settings, and Capabilities (→ this ledger) are live; Users (/admin-v3/users — account directory + CRUD) and Permissions (/admin-v3/permissions — action-level RBAC matrix from the shared PERMISSION_GROUPS catalog, persisting custom grants) are live. Compliance (/admin-v3/compliance — license/inspection expiry calendar) and Regulatory disclosures (/admin-v3/regulatory-compliance — per-site EU/NYC/SG packs) are live. Settings (/admin-v3/settings — business + storefront-layout toggles + feature flags) is live. The Calculator (/admin-v3/simulation) is live through Part 3a: its P&L compute engine was extracted to a shared pure lib (src/lib/simulation-engine.ts — computeScenario + computeTornado + computeReturns + projectMonths/projectTwelveMonths) so v3 runs identical real math; live levers drive the P&L, KPIs (margin/break-even/prime/CM1/capacity/payback) and a sensitivity tornado, saved via PUT /api/admin/simulation. Investor Returns (NPV @ 10/15/20%, bisected IRR, payback month + 24-month cumulative cash-recovery) and a seasonality×weather×inflation-composed 12-month revenue/net-profit projection chart are live too. A real-order Sandboxes card (window selector over four tabs reading live orders) is live: Cohort/LTV-CAC (/api/admin/simulation/cohorts), Dayparts (/dayparts), Hourly throughput with capacity colouring (/hourly), and Menu engineering quadrants with margin-trap/prep-heavy flags (/menu-engineering), all ?days=N. Five-section ⓘ explainers (Rule #12) are live on the six headline KPIs via a v3-native MetricExplainer/InfoButton primitive (src/admin-v3/ui/Explainer.tsx) that fixes the description → INSTITUTIONAL ANALYSIS → IN PLAIN TERMS → TIPS → METHODOLOGY order. Behaviour & environment levers fold into the headline P&L: applyAssumptions + applyAnnualWeather were extracted into the shared engine, with input cards for Behaviour assumptions (6 attach levers + combo + delivery share), Ingredient cost stress (10 per-line deltas) and Seasonality & weather (quarterly multipliers + a calibrated weather/holiday model). The fleet/franchise model + channel-economics breakdown (computeFleetEconomics/computeChannelEconomics) are now in the engine + v3 Calculator: a Channel mix & fleet input card drives the per-channel fee mix and multi-unit model, with a Channel economics table (unblended per-channel CM1) and a Fleet economics card (fleet revenue/EBITDA, avg per unit, HQ absorption, per-unit table). The v3 Calculator is at functional parity with v2 AdminSimulation. Every admin section now runs on v3. Core (POS/KDS/Guest) is untouched.",
        },
        {
          name: "Core v2 (operational suite) — rebuild in progress",
          status: "live",
          href: "/core-v2/pos",
          summary:
            "Ground-up 1:1 rebuild of the Core operational suite (POS · KDS · Guest Engagement · Service) onto the core-suite mockup (public/mockups/core-suite/), isolated under src/app/core-v2 + src/core-v2 so the live /core/* surfaces stay untouched until a swap. Core v2 is a SEPARATE ENTITY: its own self-contained theme (src/app/core-v2/styles/, every rule scoped under .corev2, design tokens on .corev2 not :root) and its own next/font — it imports NO admin or homepage theme CSS. Chrome is the mockup's left SIDEBAR shell (CoreShellV2: brand + Core/Operations nav groups + user foot from /api/admin/me, and a breadcrumb topbar) — the missing piece the no-sidebar v1 shell never matched. POS (/core-v2/pos) is live at full functional parity with /core/pos and pixel-faithful to pos.html: concurrent OPEN CHECKS tab rail (server-backed PosTab via /api/admin/pos/tabs, 350ms debounced persist + 5s live-sync), 86px category rail with capacity-true promise times, text-forward menu grid with role/dietary/pace-tag badges, the persistent coursing ticket (per-course Fire + drag-to-recourse + Coursed/All-together toggle), dine-in table picker (/api/admin/floor/tables) + covers + conflict/over-capacity warnings, delivery address, combo + cross-sell offers (getActiveComboDeals/getCartSuggestions), Pace→POS steering (/api/admin/pace/steering, 15s), Send to KDS + Charge/tender (/api/admin/pos/orders), park/hold, keyboard shortcuts (N/1–9/F/Esc) and a fullscreen kiosk (createPortal, .corev2-wrapped per rule #4). KDS (/core-v2/kds) is live too — the full-bleed dark wall ported 1:1 from kds-fleet/kds/kds-chef.html (Fleet command wall with health rings + pace gauges + promise-accuracy benchmark + per-truck tiles → Floor board with ops header/86/recall/stage filter → Chef line, role-routed, SSE live tickets via useAdminOrdersStream, bump hotkeys, fullscreen kiosk), reusing /api/admin/orders[/stream], /api/admin/kds/{fleet,floor-ops,eighty-six} and the kds-prediction/kds-ticket libs. Service (/core-v2/service) is live — Floor (live room twin + seating recommender + table CRUD over /api/admin/floor-twin + /api/admin/floor/tables) and Slots (Manage day/week + Demand yield board over /api/admin/slots + /api/admin/demand-exchange) on CoreShellV2. Guest Engagement lands next on the same separated theme; until then /core-v2/guest renders the v2 chrome with a placeholder and the live hub remains at /core/guest.",
          caveats:
            "POS · KDS · Service are live; /core-v2/guest is still a placeholder pending the next rebuild commit. The production suite remains /core/* until a swap.",
        },
        {
