  ...
  const groups: CapabilityGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          summary:
            "A pure, explainable table-assignment engine (src/lib/seating.ts, suggestTables/recommendTable — 21 unit tests) behind Service · Book. Once a slot supplies a seating time, every table is hard-filtered (capacity fit · free-for-the-full-turn using the turn-time model + reset buffer · availability) then scored 0–100 over weighted soft signals (right-size · runway comfort · guest preference/zone · pacing per 15-min bucket · yield — big tables held back from small parties). The Book table-picker's ✨ Recommend row is the engine's top pick and each row's tag + tooltip is its reason (e.g. 'held 32m', 'large table — held back for big parties', '89 pts · exact fit'); excluded tables dim. FOUR live capabilities: (1) SEAT LIFECYCLE — Today's-bookings rows carry Seat / No-show / Complete actions that transition the reservation (booked→seated→completed) and stamp seatedAt/completedAt (POST /api/admin/floor/reservations), so Book finally answers 'who's at T5?'. (2) WALK-IN GUARD — a '+ Walk-in' dialog ranks tables at NOW with the same engine and only lets you seat a genuinely-free one (reserved-soon tables dim), writing an ad-hoc source:'walk-in' seated reservation so the floor counts it. (3) MANAGER POLICY — a '⚙ Policy' dialog with presets (balanced / maximise-covers / guest-first / slow-night) + weight sliders (fit/runway/guest/pacing/yield) + rules (reset buffer / pace cap / large-table seats), persisted per location (GET/PUT /api/admin/seating/policy, saves instantly per Rule #7). (4) LEARNED TURN-TIMES — expectedTurnMin uses a shrinkage-smoothed model derived live from completed reservations' seatedAt→completedAt spans (GET /api/admin/seating/turn-model), sharpening the free-window guard as parties actually close; cold-starts on party-size defaults (Rule #1 — no fabricated data). Pure & client-safe (reuses floor.ts, no I/O). Additive throughout — falls back to a plain capacity check before a slot is chosen, so nothing regresses. Deep-dive: tests/sketches/host-06-engine.html.",
        },
        {
