...
export default async function CorePosPage() {
  ...
  const [locations, upsell, operator] = await Promise.all([getActiveLocationsAsync(), getUpsellSettings(), getCurrentAdminUser()]);
  ...
  return <CorePos menusByLocation={menusByLocation} upsellByLocation={upsellByLocation} operatorName={operator?.name} />;
}
