...
| **§1.4 row 5 — SG NEA Nutri-Grade** | Wired but operator-typed per beverage. | Slight improvement only. Nutri-Grade A–D bands depend on total/saturated/added sugar thresholds; the macro pipeline now stores per-100g sugar + saturated fat per active offering, so an automatic Nutri-Grade computation from the recipe nutrition is now feasible — schema is ready, the computation function is not yet written. **Still operator-typed today**; the automation is one more commit away. |
...
| Singapore viability | 29/100 | **30/100** | Nutri-Grade computation is structurally feasible from the recipe nutrition; not yet written. Halal cert + MUIS banner + pork/alcohol chips unchanged. |
...
1. **NEA Nutri-Grade computation** from per-100g sugar + saturated fat is one commit away. Schema is ready. Not written.
