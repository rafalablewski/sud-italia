...
2. "Sidebar is a flat list of 25 items" — `src/components/admin/v2/nav.config.ts` already groups items into 10 named sections (adds a "Core" group: POS/KDS/CRM/Concierge/WhatsApp, `nav.config.ts:92`) — Overview / Core / Operations / Inventory / People / Customers / Finance / Growth / Intelligence / System — and role-based filtering is real (`requiredRole` per item). Marked out of date in §2.1 and §2.2 #1.
...
1. **The sidebar is a flat list of 25 items** masquerading as 12 sections. A line cook will never use `/admin/expansion` or `/admin/ai`; an owner rarely uses `/admin/kds`. — ✓ Out of date: `src/components/admin/v2/nav.config.ts` declares 10 named sections (adds a "Core" group: POS/KDS/CRM/Concierge/WhatsApp, `nav.config.ts:92`) — Overview / Core / Operations / Inventory / People / Customers / Finance / Growth / Intelligence / System — and role-based filtering is now real (`requiredRole` per item) so the nav collapses by role.
...
| Proposal | Status |
|---|---|
| 5-zone IA (`NOW / PEOPLE / KITCHEN / TEAM / BUSINESS`) | ✗ Not fixed — current nav uses a similar but different 10-section grouping (`nav.config.ts:92`); zone collapse + `Cmd+1..5` not wired |
...
Consistent with this audit's discipline of flagging cosmetic-not-functional surfaces: the V8 `/rewards` rebuild ships a loyalty **streak** (hardcoded "2", `rewards/page.tsx:459`), a **weekly challenge** (hardcoded "33% / 1-of-target", `:482-486`), and a customer-facing **referral code generated with `Math.random()`** (`generateReferralCode`, `growth-engine.ts:18`) each render (`src/app/(public)/rewards/page.tsx`) — none wired to the real `referral-loop.ts` persistence. The root is `getActiveChallenges()` (`growth-engine.ts:126`) returning a static hardcoded array with no per-customer progress field, so the bar is structurally incapable of reflecting real progress; `simulateLiveActivity()` (`growth-engine.ts:175-182`) likewise fabricates the live-ticker counts with `Math.random()`. These violate CLAUDE.md Rule #1 and should be wired to real data. Two storefront surfaces (`/review/[orderId]`, `/corporate/[slug]`) also remain on the pre-V8 `italia-*` palette — design-system drift under the new Rule #11.
