...
| 3 | Refund × bundle correlation | ⏳ Not shipped. `OrderRefund` reason-code enum still absent; no admin reason-picker on the refund button (the audit-3 row in the institutional-grade audit's Appendix A still reads "Refund × bundle correlation" as open). |
