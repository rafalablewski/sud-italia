...
- [`components.md`](./components.md) — `.pub-*` form primitives; shared `<Button>` / `<Sheet>` / `<Container>` / `<StarRating>` / `<CurrencySwitcher>`; landing sections; item card; `<CartDrawer>` and `<FloatingCartButton>`; `<DeliveryProgress>`; the `/rewards` chrome (`.v8-rewards-*`); `<OrderTracker>`.
