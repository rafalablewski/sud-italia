...
- **Theme:** `.core-surf-toolbar` (ActionBar, with a `.core-slot-selbtn`
  Select toggle) · `.core-statstrip` · `.core-surge-banner` ·
  `.core-slot-bulkbar` (`.bb-txt` / `.bb-btn.on|.off` / `.bb-clear`, shown in
  select mode) · `.core-slots-grid` (two-column) · `.core-frame` /
  `.core-frame-h` / `.core-frame-b` · Manage rows `.core-mslot` (`.barwrap` /
  `.mbar` fill + `.meta` + `.core-tchip` tier chip + `.mcap` + `.core-avail`
  availability pill; `.selectable`/`.sel` + `.mslot-check` in select mode) ·
...
- **Manage** and **Demand exchange** are shown **side by side** in
  ...
  - **Manage · service windows** — each `.core-mslot` is a capacity fill bar
    (basil→amber→danger ≥85%), a booked/status meta line, a display-only
    **tier chip** (healthy · tight · full), `N / max`, and a **`.core-avail`
    availability pill** (basil *Available* ↔ muted *Unavailable*) that
    flips the window active↔draft and persists immediately (Rule #7). For any
    window **serving dine-in** the row uses the *seating* lens: the fill bar,
    `N booked` meta and `N / max` count **tables held by real reservations** at
    that time (Book's occupancy — a booking holds its table for its 90-min stay
    **+ 15-min cleanup turnaround**, so it counts against every window it spans),
    against a ceiling of the floor's **table count** — shown as **"seats to
    &lt;tables&gt;"**. This is *not* `slot.maxOrders` / `currentOrders`, which are
    the separate online/POS order-throughput cap + tally that reservations never
    touch (see `src/lib/booking.ts`). **Online-only** windows keep the order
    lens: `currentOrders / maxOrders` and **"orders to &lt;maxOrders&gt;"**. (The
    6-up stat strip above stays the order/forecast aggregate, paired with the
    Demand Exchange.)
    **Dine-in windows are auto-generated by default** — a slot every 15 min
    across the location's open hours for the whole floor (capacity = table
    count), so a day is always fully reservable without hand-building it;
    closing one = mark it *Unavailable* (persisted draft, hidden from Book),
    never delete (`dine-…`-id grid slots are recreated on load, so they carry
    no delete affordance — only manual slots do). The toolbar **Select**
    toggle (`.core-slot-selbtn`) enters multi-select: tap rows to check them
    (`.mslot-check`), then the `.core-slot-bulkbar` closes/opens the whole set
    in one PUT (e.g. block an evening for a private event). Week mode groups
    the seven days under `.core-slot-day` headers.
...
- **Engine:** `GET /api/admin/slots?location=[&date=][&ensureDineIn=1]` (day =
  date-scoped; `ensureDineIn=1` first materialises the default 15-min dine-in
  grid for that day via `ensureDineInSlots` in `src/lib/store.ts` — idempotent,
  deterministic `dineInSlotId(...)` ids, capacity synced to the live table
  count; week = whole location sliced client-side) +
  `GET /api/admin/demand-exchange?location=&date=` (forecast); toggle /
  bulk-availability = `PUT /api/admin/slots` (`{ id, status }` single /
  `{ ids, status }` bulk); create = `POST /api/admin/slots` (single) /
  ...
  Book pulls the same grid with `&ensureDineIn=1` so its reservation slots are
  always present.
...
- **Slots** — leading `Manage|Demand` seg (brand-active) · Day/Week seg · the shared `CoreDateField` picker (left); on the right a `CoreFilterMenu` funnel (Fulfillment: All · Dine-in · Takeaway · Delivery) · a `.core-slot-selbtn` **Select** toggle (multi-select availability) · standard `.core-iconbtn` Refresh · orange New-slot pill (`.core-slot-add`); stat cells 5–6 are Covers booked (info) + No-show risk (danger, flagged); default `.delta` basil/green; Manage tier chips fixed 46px.
