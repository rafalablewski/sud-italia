...
- **Manage** and **Demand exchange** are shown **side by side** in
  ...
  - **Manage · service windows** — each `.core-mslot` is a capacity fill bar
    ...
    **tier chip** (healthy · tight · full), `N / max`, and a **`.core-avail`
    ...
    flips the window active↔draft and persists immediately (Rule #7). The
    status line's capacity reads **"seats to &lt;tables&gt;"** for any window
    serving dine-in — the floor's **table count** is the real dine-in seat
    ceiling (one party per table), *not* `maxOrders`, which is the separate
    online/POS order-throughput cap (see `src/lib/booking.ts`). Online-only
    windows instead read **"orders to &lt;maxOrders&gt;"**. (The 90-min turn +
    15-min cleanup hold that governs live availability is applied per-reservation
    in Book's slot fill; this line is the static ceiling.)
