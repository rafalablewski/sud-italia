...
- **Layout:** dense-console (mockup 07-guest-inbox) — the `.core-surf-toolbar`
  ActionBar, then a **5-up `.core-statstrip`** (open
...
`src/core/guest/CoreCrm.tsx`. Rendered in the **dense-console** language: the `.core-surf-toolbar` ActionBar, then a **6-up `.core-statstrip`** — **guests · VIPs · new · at-risk · avg spend · repeat rate** (all derived from the live book: at-risk = RFM health < 34, repeat = 2+ orders; coloured values + mono deltas, Rule #1).
- **ActionBar** (`.core-surf-toolbar` via `CoreSurfToolbar`): a `.core-crm-search` on the left; on the right the **Segment · Tier · Sort** filters collapse into ONE **`CoreFilterMenu`** funnel popover (the Guest Inbox header-tool treatment — `.core-filt-btn` with an active-count badge) followed by refresh. The popover groups: **Segment** with live counts (All · VIP · Regular · New · At-risk — see `inSeg`/`segCounts`), **Loyalty tier** with the metal `.core-gem` dot (Bronze · Silver · Gold · Platinum, clearable → `tierF`), and **Sort by** (Recent · Spend · Visits → `recent`/`ltv`/`orders`, a preference, not badged).
