...
- **ActionBar** (`.core-surf-toolbar` via `CoreSurfToolbar` — the `.core-surf-id` context anchor left): its filters use the **same labelled `.core-seg` capsule vocabulary as every other surface** — a **`Segment`** seg with live `.c` counts (All · VIP · Regular · New · At-risk — see `inSeg`/`segCounts`) and a **`Tier`** seg whose options carry the metal `.core-gem` dot (Bronze · Silver · Gold · Platinum, toggling `tierF`), preceded by a `.core-crm-search`; on the right a **`Sort`** `.core-seg` (recent · spend · visits → `recent`/`ltv`/`orders`) and refresh.
