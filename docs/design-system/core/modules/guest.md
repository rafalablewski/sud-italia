...
- **Layout:** dense-console (mockup 07-guest-inbox) — the `.core-surf-toolbar`
  ActionBar (identity `Guest · Inbox`), then a **5-up `.core-statstrip`** (open
...
`src/core/guest/CoreCrm.tsx`. Rendered in the **dense-console** language: the `.core-surf-toolbar` ActionBar (identity `Guest · CRM`), then a **6-up `.core-statstrip`** — **guests · VIPs · new · at-risk · avg spend · repeat rate** (all derived from the live book: at-risk = RFM health < 34, repeat = 2+ orders; coloured values + mono deltas, Rule #1).
- **ActionBar** (`.core-surf-toolbar` via `CoreSurfToolbar` — the `.core-surf-id` identity left): as its `left` a `.core-crm-search`, **labelled segment chips** `.core-segchips` with live counts (All · VIP · Regular · New · At-risk — see `inSeg`/`segCounts`) and **loyalty-tier gems** `.core-gems`/`.core-gemchip` (Bronze · Silver · Gold · Platinum, toggling `tierF` via `gemClass`); on the right a **sort** `.core-seg` (recent · spend · visits → `recent`/`ltv`/`orders`) and refresh.
