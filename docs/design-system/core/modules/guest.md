...
- **ActionBar** (`.core-surf-toolbar` via `CoreSurfToolbar` — the `.core-surf-id` context anchor left): as its `left` a `.core-crm-search`, **labelled segment chips** `.core-segchips` with live counts (All · VIP · Regular · New · At-risk — see `inSeg`/`segCounts`) and **loyalty-tier gems** `.core-gems`/`.core-gemchip` (Bronze · Silver · Gold · Platinum, toggling `tierF` via `gemClass`); on the right a **sort** `.core-seg` (recent · spend · visits → `recent`/`ltv`/`orders`) and refresh.
