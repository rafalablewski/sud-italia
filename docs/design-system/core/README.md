...
| **Layout** | The Service OS three-region IA: a thin **Command Bar** (top) · the **Lens Rail** (left) · the surface **Canvas** (right) · the persistent **Context Dock** (docked bottom) | One chrome shared by every surface. The Command Bar is the **"Command"** terminal row (all-monospace): traffic-light chrome · a **live shell prompt** stating exactly where you are (`core ❯ pos:order`) with a blinking caret · the surface's mono view-tab chips · the surface's own controls · a **⌘K** launcher · a **risk · loc · clock** telemetry cluster · global tools (**notifications bell** · theme). The **Lens Rail** (`.core-lens`, left, icon-only 60px, expands to labels only when **pinned** — a click on the pin toggle, never on hover; the choice persists) switches *how you view the room* — the four room lenses **Floor · POS · KDS · Book** (the plain names — not "Line"/"Pass"); Orders and Guest are cross-cutting surfaces reached from ⌘K, not room lenses. The selected entity persists across lenses and its check stays docked. |
