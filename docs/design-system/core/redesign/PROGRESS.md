...
| 4 | `<ServiceCanvas>` shell + Lens Rail + Command Bar | ◐ | Bottom-nav lens switch exists; command bar reused. **Role-shaped default lens shipped:** `/core` now redirects by session role (`getCurrentRole`) — **kitchen → Pass (KDS)**, everyone else → Floor home base (finer FOH titles collapse to the "staff" tier, so can't be split further server-side). **PressureBadge shipped:** a live command-bar load indicator on every lens (`.core-pressure` in `CoreShell` — "{n} at-risk · line {m} · {k}m wall" from `GET /api/admin/pos/pressure`, the same predictive tier the KDS colours from), pulsing on warn/risk. Still open: `⌘K` palette, Core `/handover`. |
