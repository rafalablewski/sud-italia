...
| 7 | Line lens (POS) on the shared dock | ◐ | **Real-time 86 shipped:** sold-out items (base-unavailable OR live-86'd) are no longer filtered out — they stay on the grid **greyed + struck (`.core-prod.sold-out`, `.core-tag.off` "86 · sold out") and sink to the bottom**, non-tappable, so the line never taps a gap but always sees it. Live via a 15s poll of `/api/admin/kds/eighty-six` (the kitchen's authoritative override list) — no reload. Verified live (86'd Diavola sank + greyed, screenshotted). Remaining Line-lens items: dock `<CourseSpine>`, ★Popular/Smart category, smart-default fire, fire-moment upsell (milestone 10). |
