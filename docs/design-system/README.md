...
| **Per-route bundle loading**  | ✅ `src/app/(public)/layout.tsx` imports `themes/homepage/index.css`; `src/app/admin/layout.tsx` imports `themes/admin/index.css`; the Core suite's `src/app/core/layout.tsx` imports `themes/admin/index.css` + `themes/core/index.css` + `themes/core/suite.css` (Core reuses the admin tokens, then layers `.core-suite` / `.kds-core` on top). Storefront pages no longer ship admin's or core's chunk; the `/admin` back-office no longer ships core's chunk (it left when POS / KDS / Guest / Service moved to `/core/*`); admin pages no longer ship homepage's chunk. The Homepage `tokens.css` (~50 lines, @theme inline only) ships globally — that's the Tailwind v4 utility-generation cost. |
