...
public let OPERATOR_NAV: [OperatorNavSection] = [
    OperatorNavSection("core", "Core", [
        ...
        OperatorNavItem("/core/guest", "Guest Engagement", "person.2.fill", .staff, .scaffold,
                        "Bookings, concierge, guest profiles, loyalty and inbox."),
        OperatorNavItem("/core/service", "Service", "fork.knife.circle.fill", .staff, .scaffold,
                        "Floor plan, table turns and reservation slots."),
    ]),
    OperatorNavSection("overview", "Overview", [
        OperatorNavItem("/admin/welcome", "Welcome", "sparkles", .kitchen, .scaffold,
                        "Day-one tour and quick links into the operation."),
        OperatorNavItem("/admin", "Dashboard", "rectangle.3.group.fill", .kitchen, .scaffold,
        ...
        OperatorNavItem("/admin/alerts", "Alerts", "bell.fill", .staff, .scaffold,
                        "Stock-outs, late tickets, SLA breaches and ops nudges."),
        OperatorNavItem("/admin/comms/tasks", "Tasks", "checklist", .manager, .scaffold,
                        "Shift checklists and assigned to-dos with sign-off."),
        OperatorNavItem("/admin/comms/announcements", "Announcements", "megaphone.fill", .manager, .scaffold,
                        "Broadcast notices to the floor and kitchen."),
    ]),
    OperatorNavSection("operations", "Operations", [
        OperatorNavItem("/admin/menu", "Menu", "menucard.fill", .manager, .scaffold,
                        "Per-location menu, availability and 86-ing."),
        OperatorNavItem("/admin/recipes", "Recipes", "flask.fill", .manager, .scaffold,
        ...
        OperatorNavItem("/admin/waste", "Waste log", "trash.fill", .staff, .scaffold,
    ...
    OperatorNavSection("inventory", "Inventory", [
        OperatorNavItem("/admin/inventory", "Stock", "shippingbox.fill", .staff, .scaffold,
                        "On-hand counts, par levels and variance."),
        OperatorNavItem("/admin/suppliers", "Suppliers", "building.2.fill", .manager, .scaffold,
                        "Vendor catalogue, lead times and pricing."),
        OperatorNavItem("/admin/purchase-orders", "Purchase orders", "doc.text.magnifyingglass", .manager, .scaffold,
                        "Raise, receive and reconcile POs against par."),
    ]),
    OperatorNavSection("people", "People", [
        OperatorNavItem("/admin/staff", "Staff", "person.text.rectangle.fill", .manager, .scaffold,
                        "Team roster, roles and time punches."),
        OperatorNavItem("/admin/schedule", "Schedule", "calendar.badge.clock", .manager, .scaffold,
                        "Rota planning against forecast demand."),
    ]),
    OperatorNavSection("customers", "Customers", [
        OperatorNavItem("/admin/customers", "Customers", "person.crop.circle.fill", .staff, .scaffold,
        ...
        OperatorNavItem("/admin/feedback", "Feedback", "bubble.left.and.bubble.right.fill", .manager, .scaffold,
                        "Reviews and sentiment, with AI summaries."),
        OperatorNavItem("/admin/surveys", "Pulse surveys", "gauge.medium", .manager, .scaffold,
                        "Guest pulse surveys and response analytics."),
    ]),
    OperatorNavSection("finance", "Finance", [
        OperatorNavItem("/admin/reports", "Reports", "chart.bar.fill", .manager, .scaffold,
                        "Sales, cohort, delivery, tips and JPK exports."),
        OperatorNavItem("/admin/cash", "Cash", "banknote.fill", .manager, .scaffold,
                        "Till counts, drops and reconciliation."),
        OperatorNavItem("/admin/business-costs", "Business costs", "wallet.bifold.fill", .manager, .scaffold,
    ...
    OperatorNavSection("growth", "Growth", [
        ...
        OperatorNavItem("/admin/upsell", "Upsell", "arrow.up.forward.circle.fill", .manager, .scaffold,
                        "Cart upsell rules and performance."),
        OperatorNavItem("/admin/crosssell", "Cross-sell", "sparkles", .manager, .scaffold,
                        "Pairings — espresso + dessert with every pizza."),
        OperatorNavItem("/admin/scheduled-bundles", "Scheduled bundles", "calendar", .manager, .scaffold,
                        "Time-boxed set-price combos."),
        OperatorNavItem("/admin/events", "Events & bookings", "calendar.badge.plus", .manager, .scaffold,
                        "Private events and large-party bookings."),
        OperatorNavItem("/admin/integrations", "Integrations", "powerplug.fill", .manager, .scaffold,
                        "Aggregators, WhatsApp, payments and webhooks."),
    ]),
    OperatorNavSection("intelligence", "Intelligence", [
        OperatorNavItem("/admin/locations", "Multi-location", "map.fill", .owner, .scaffold,
        ...
        OperatorNavItem("/admin/ai", "Insights", "brain.head.profile.fill", .manager, .scaffold,
        ...
        OperatorNavItem("/admin/expansion", "Expansion", "map", .owner, .scaffold,
                        "Site selection and new-market modelling."),
    ]),
    OperatorNavSection("system", "System", [
        OperatorNavItem("/admin/users", "Users & roles", "checkmark.shield.fill", .owner, .scaffold,
        ...
        OperatorNavItem("/admin/compliance", "Compliance", "calendar.badge.checkmark", .manager, .scaffold,
        ...
        OperatorNavItem("/admin/audit-log", "Audit log", "clock.arrow.circlepath", .manager, .scaffold,
        ...
        OperatorNavItem("/admin/payments", "Payments", "creditcard.fill", .manager, .scaffold,
                        "Stripe, terminals and payout configuration."),
        OperatorNavItem("/admin/qr-ordering", "QR ordering", "qrcode", .manager, .scaffold,
                        "Table QR codes and at-table ordering."),
        OperatorNavItem("/admin/currency", "Currency", "coloncurrencysign.circle.fill", .owner, .scaffold,
                        "Display currency and FX configuration."),
        OperatorNavItem("/admin/languages", "Languages", "globe", .owner, .scaffold,
                        "Storefront locales and translations."),
        OperatorNavItem("/admin/settings", "Settings", "gearshape.fill", .owner, .scaffold,
