...
The platform installs as **two distinct home-screen PWAs** (tablets, iPads,
iPhones), driven entirely by per-route metadata — there is one app, one
deploy, one service worker:

- **Ottaviano** — the customer app: storefront ordering + the Soci e amici
  loyalty card. Portrait, brand-red, served by the root layout
  (`/manifest.json`). Installs from any storefront route; an in-page Install
  button lives on `/rewards`.
- **OttavianoKDS** — the operator app: full Admin + Core (POS, KDS, Orders,
  Guest, Service). Landscape-first, dark, served by the admin / core / kitchen
  / login / `/operator` layouts (`/ottaviano-kds.webmanifest`). Its home is
  **`/operator`** — an auth-gated launcher into every operator surface.

The split is the single source of truth in `src/lib/pwa.ts`: customer routes
advertise the Ottaviano manifest + apple title, operator routes spread
`kdsAppMetadata` to override that subtree with the KDS identity. Icons are
generated by `scripts/gen-app-icons.ts` into `public/icons/{ottaviano,kds}/`.
...
│   ├── icons/            App icons: ottaviano/ (customer) + kds/ (operator)
│   ├── sw.js             Service worker (serves both app shells)
│   ├── manifest.json     Ottaviano (customer) PWA manifest
│   └── ottaviano-kds.webmanifest  OttavianoKDS (operator) PWA manifest
├── scripts/
│   ├── chaos-phase0.ts   Chaos harness (npm run chaos)
│   ├── gen-app-icons.ts  Regenerate the two apps' home-screen icons
