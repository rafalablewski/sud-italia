...
| P9-10 | Marketing | `VIP_SPEND_GROSZE`, `VIP_ORDERS` | `whatsapp/audience.ts:21-22` | a VIP-definition setting | P2 | **DEFERRED-by-design.** The broadcast audience cut (spend + order count) is a different axis from the loyalty tier ladder (points); "aligning" the two isn't clearly correct, and a separate VIP setting is marketing tuning that can wait. |
...
| P9-12 | Brand | JPK `JPK_NIP/NAME` env placeholders | `jpk.ts:73-74` | admin compliance config | P2 | **DEFERRED.** Acceptable env bootstrap (documented placeholder). A future `legalEntity` settings block alongside `vatRateBps` would fold it in; low urgency. |
