...
12. **Every metric & lever explains itself in five parts.** Any ⓘ `InfoButton` on a KPI card, metric, or what-if lever (simulators, sandboxes, reports, the Calculator — anywhere a number or control needs explaining) MUST render all five sections, in this exact order, with these exact labels: (1) a one-line **description**, (2) **INSTITUTIONAL ANALYSIS** (the analyst/CFO framing — benchmarks, what "good" looks like, the institutional gate), (3) **IN PLAIN TERMS** (storytelling with a concrete złoty example), (4) **TIPS — HOW TO PUSH THIS LEVER** (concrete operator actions to move the number), (5) **METHODOLOGY — HOW THIS IS DETERMINED** (the formula + data source). Build it from `MetricExplainer` in `src/components/admin/Explainers.tsx` — it fixes the order and labels, and its five props are all required so a half-written explanation won't compile. Never ship a metric ⓘ with only a description, and never invent ad-hoc section labels — extend `MetricExplainer` instead. No section may be empty or hand-waved ("self-explanatory" is not an explanation).
