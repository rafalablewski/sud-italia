...
| `crm/` | Customer Relationship Management — the chosen direction, **Regulars** (searchable customer book beside a deep relationship profile), browsable from `index.html`. Reuses the POS + KDS visual language; runs end to end (search, segment, sort, RFM health score, loyalty tier, AI next-best-action, points, consent toggles, notes) over a 12-customer sample book in złoty. |
