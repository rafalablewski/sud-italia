...
// Swift IA for the OttavianoKDS app (native/ottaviano-ios). The overlay's SF
// Symbols feed this directly (they were authored for the SwiftUI app); the RN
// skin ignores them and maps its own MaterialCommunityIcons.
...
// ── Emit the generated TypeScript (the OPERATOR_NAV data the RN app renders) ──
// Structure/roles/blurbs/state are generated from the web source; the native
// presentation icon (a MaterialCommunityIcons name) is applied in operatorNav.ts,
// keyed by path — it has no web-CSS/overlay equivalent (the overlay's SF Symbols
// don't map to RN), so it stays a thin presentation layer over this data.
function tsStr(s: string): string {
  return '"' + s.replace(/\\/g, "\\\\").replace(/"/g, '\\"') + '"';
}
function tsItem(i: Item): string {
  return (
    `    { path: ${tsStr(i.href)}, label: ${tsStr(i.label)}, ` +
    `requiredRole: ${tsStr(i.role)}, status: ${tsStr(i.kind)}, blurb: ${tsStr(i.blurb)} },`
  );
}
function tsSection(s: Section): string {
  return (
    `  { id: ${tsStr(s.id)}, label: ${tsStr(s.label)}, items: [\n` +
    s.items.map(tsItem).join("\n") +
    `\n  ] },`
  );
}
const swift = `// @generated by scripts/gen-native-nav.ts — DO NOT EDIT.
...
${sections.map(tsSection).join("\n")}
];
`;

// ── Emit the generated Swift (the OPERATOR_NAV the OttavianoKDS app renders) ──
// The overlay's SF Symbols become the native glyphs directly; types, the role
// enum, and `filteredNav(for:)` live in OperatorNav.swift.
...
const swiftNav = `import Foundation
...
const outputs: { path: string; content: string }[] = [
  ...
  { path: TS_OUT, content: swift },
  { path: SWIFT_OUT, content: swiftNav },
